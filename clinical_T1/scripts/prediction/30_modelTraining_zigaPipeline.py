...
ssh skatz@212.235.237.86 -p 62636 'source miniconda3/bin/activate sklearn | /home/skatz/miniconda3/envs/sklearn/bin/python /home/skatz/PROJECTS/covid/disease_severity/discoveryValidation/clinical_T1/scripts/prediction/30_modelTraining_zigaPipeline.py'
'''
...
PATH_base = "/home/skatz/PROJECTS/covid/disease_severity/discoveryValidation/clinical_T1/"
...
''' 
Select features --> change here for different setups!
'''
vars = "manualSelection/rmUnclearVars"    # clinical+bestSterols
...
try: 
      ...
      #model = "log"
            ...
            clf = GridSearchCV(models[model], grids[model], scoring='balanced_accuracy', verbose=1, cv=3, n_jobs=-1)  ##changed this for svc to cv=2; otherwise takes too long!
            result = classify_leave_one_out_cv(clf, 
                                    ...
                                    logfile = "log_clinicalT1_allModels")
            ''' Prepare results '''
            result['model'] = model        
